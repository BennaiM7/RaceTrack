...
def solveur(trajectoire, visite, choix_map, taille_case_x, taille_case_y, ligne_lst):
    ...
    Fonction qui cherche une trajectoire du départ à l'arrivé et affiche les mouvements en temps réel.
    """
...
if __name__ == "__main__":
    ...
    while boucle:
        ...
        texte(500,650,"Solver",ancrage="center",couleur="black")
        ...
        while menu:
            ...
            if tev == 'ClicGauche':
                ...
                x = position_l[-1][0]
                y = position_l[-1][1]
                if position_l[-1][0] >= 400 and position_l[-1][0] <= 600 and position_l[-1][1] >= 400 and position_l[-1][1] <= 600:
                    menu = False
            elif tev == 'Quitte':
                    break
            ...
            if tev == 'ClicGauche':
                ...
                if nom_touche == 'a':
                    ...
                    for i in range(len(trajectoire)-1):
                        point_actuel = trajectoire[i]
                        point_suivant = trajectoire[i+1]
                        ligne_lst.append((taille_case_x*point_actuel[0], taille_case_y*point_actuel[1], taille_case_x*point_suivant[0], taille_case_y*point_suivant[1]))
                    for elem in ligne_lst:
                        ligne(elem[0], elem[1], elem[2], elem[3], epaisseur="2")
            elif tev == 'Quitte':
                ferme_fenetre()
                break
            
            if option == []:
                texte(400,400, "Vous avez perdu!", couleur="red")
                boucle = False
                break 
